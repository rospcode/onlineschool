...
class AdminRequestsController extends Controller
{
  ...
  if($request->email != null){
   ...
   public function marksave($id){
     ...
     $getmarks = graduates::where("courses_id",$id)->get();
     $message = "";
     foreach ($get as $temp) {
       if(count($getmarks) > 0){
          foreach ($getmarks as $marks) {
           if($temp->user_id == $marks->user_id){
             $message = 'cannot continue, some students already have marks';
             $feeddelte = TempMarks::where("user_id",$temp->user_id)->first();
             $deleteenrolled = Enrolled::where("user_id",$temp->user_id)->first();
             if($deleteenrolled != null){
                $deleteenrolled->delete();
             }
            if($feeddelte != null){
               $feeddelte->delete();
            }
             ...
             $allocate = graduates::create([
                'user_id' => $temp->user_id,
                'courses_id' => $temp->courses_id,
                'course_marks' => $temp->course_marks
             ]);
             $feeddelte = TempMarks::where("user_id",$temp->user_id)->first();
             $deleteenrolled = Enrolled::where("user_id",$temp->user_id)->first();
             if($deleteenrolled != null){
            ...
            if($feeddelte != null){
             ...
             $message = 'done uploading marks, skipped users who already had marks';
           }
         }
       }else{
         $allocate = graduates::create([
         ...
         $feeddelte = TempMarks::where("user_id",$temp->user_id);
         $deleteenrolled = Enrolled::where("user_id",$temp->user_id);
         if($deleteenrolled == null){
            $deleteenrolled->delete();
         }
        if($feeddelte == null){
   ...
   public function deleteuser($id){
    ...
    public function approveuser($id){
     ...
     Mail::send('controllemails/welcome', $data, function($message) use ($data){
        ...
        $message->from('info@capability.co.za','CapabilityHr');
     });
